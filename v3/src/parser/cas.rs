...
use nom::{
    ...
    character::complete::{digit1, i64, line_ending, space1, u64},
    combinator::opt,
    IResult, Parser,
};

pub fn make_cas_parser<'a>(
) -> impl Parser<&'a [u8], Output = Command, Error = nom::error::Error<&'a [u8]>> {
    fn cas_parser(input: &[u8]) -> IResult<&[u8], Command, nom::error::Error<&[u8]>> {
        let (input, _) = tag("cas")(input)?;
        ...
        let (input, _) = space1(input)?;
        let (input, cas_unique) = u64(input)?;
        ...
        let command: Command = Command::Cas {
            key,
            flags,
            timeout,
            noreply,
