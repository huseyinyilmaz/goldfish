...
pub fn make_prepend_parser<'a>(
) -> impl Parser<&'a [u8], Output = Command, Error = nom::error::Error<&'a [u8]>> {
    fn prepend_parser(input: &[u8]) -> IResult<&[u8], Command, nom::error::Error<&[u8]>> {
        let (input, _) = tag("prepend")(input)?;
        ...
        let command: Command = Command::Prepend {
            key,
            flags,
            timeout,
            noreply,
